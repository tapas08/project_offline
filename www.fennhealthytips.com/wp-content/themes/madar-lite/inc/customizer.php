...
//  Top Advertisement Image
    ...
    //Breaking News
    $wp_customize->add_setting('breaking_enable', array(
            'sanitize_callback' => 'madarlite_sanitize_checkbox',
            'capability'        => 'edit_theme_options',
        )       
    );
    $wp_customize->add_control('breaking_enable',array(
            'type' => 'checkbox',
   ...
   $wp_customize->add_setting('madarlite_search_icon_in_menu', array(
      'priority' => 3,
      'default' => 0,
      'capability' => 'edit_theme_options',
      'sanitize_callback' => 'madarlite_sanitize_checkbox'
   ...
   $wp_customize->add_control('madarlite_search_icon_in_menu', array(
      'type' => 'checkbox',
      'label' => __('Check to display the Menu Search Icon in the primary menu', 'madar-lite'),
      'section' => 'header-settings'
   ));
...
function madarlite_registers() {

	wp_enqueue_script( 'madarlite_customizer_script', get_template_directory_uri() . '/js/madarlite_customizer.js', array("jquery"), '20160721', true  );
	
	wp_localize_script( 'madarlite_customizer_script', 'madarLiteCustomizerObject', array(
		'pro' => __('View PRO version','madar-lite')

	) );
}
add_action( 'customize_controls_enqueue_scripts', 'madarlite_registers' );
add_action( 'wp_head', 'madarlite_style_styles' );
function madarlite_style_styles() {
	$theme_color = esc_attr(get_theme_mod('primary_color', '#007fff' ));
	?>
  <style type="text/css">
        ...
        a.random,a.user_profile,a.logout,a.login,.share-search,.top-menu li.current-menu-item,.top-menu li.current-menu-item:hover,.primary-navigation-wrap {border-bottom: 3px solid <?php echo $theme_color;?>; }
		.advertisement_in_front .widget-title {border-bottom: 2px solid <?php echo $theme_color;?>; }
        ...
        .normal-box .normal-readmore a,.one-column li.child-text .child-title h3 a:hover,.one-column li.child-news .child-title h3 a:hover,.two-column li.child-news h3 a:hover,.top-navigation select,.main-navigation select ,.site-title a,.site-title a:hover{ color:<?php echo $theme_color;?>; }
...
.main-navigation-menu a:active, .advertisement_in_front .widget-title span {background-color: <?php echo $theme_color;?>; }
