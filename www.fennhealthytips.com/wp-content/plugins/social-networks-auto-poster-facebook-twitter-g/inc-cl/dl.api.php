...
if (!class_exists("nxs_class_SNAP_DL")) { class nxs_class_SNAP_DL {
    ...
    function doPostToNT($options, $message){ $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>'');
      ...
      $advSet = nxs_mkRemOptsArr($hdrsArr, '', $flds); $cnt = nxs_remote_post( 'http://del.icio.us/login', $advSet ); 
      ...
      $advSet = nxs_mkRemOptsArr($hdrsArr, $ck); $cnt = nxs_remote_get( 'http://del.icio.us/save/get_iframe_savelink?url=&title=&notes=', $advSet ); 
      ...
      $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, $flds);  $cnt = nxs_remote_post( 'http://del.icio.us/save/bookmark', $advSet );// prr($advSet);
